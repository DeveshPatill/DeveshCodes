...
import { teachingSubjects } from '@/data/portfolio';
...
export function LearnWithMe() {
  ...
  return (
    <Section id="learn" variant="alternate" size="xl" aria-labelledby="learn-heading">
      <div className={cn('space-y-16', isVisible ? 'animate-fade-in-up' : 'opacity-0')}>
        <div className="text-center max-w-3xl mx-auto space-y-6">
          ...
          <h2 id="learn-heading" className="text-3xl sm:text-4xl lg:text-5xl font-bold text-zinc-950 dark:text-white leading-tight">
            Private Coding <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">Sessions</span>
          </h2>
          <p className="text-lg sm:text-xl text-zinc-600 dark:text-zinc-300 leading-relaxed max-w-2xl mx-auto">
            I provide private coding sessions for students who want to understand programming clearly, practice with guidance, and build a strong foundation through hands-on learning.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {teachingSubjects.map((subject, index) => (
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-zinc-950 dark:text-white mb-3">{subject.title}</h3>
              <p className="text-zinc-600 dark:text-zinc-400">{subject.description}</p>
            </Card>
          ))}
        </div>
        ...
        <div className="pt-8 border-t border-zinc-200 dark:border-zinc-700">
          <div className="text-center">
            <p className="text-lg text-zinc-600 dark:text-zinc-300 mb-6 max-w-2xl mx-auto">
              Want to learn coding one-on-one?
            </p>
